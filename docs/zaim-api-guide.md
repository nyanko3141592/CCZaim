...
Zaim API の公式ドキュメントはログイン後にしか見られず、情報が散らばっていて全体像を掴みにくい。そこで、**全エンドポイントを実際に叩いて検証した結果**を元に、「何ができて、何ができないのか」を一つの記事にまとめました。
...
Zaim に乗り換えた最大の理由は**API があること**。Web UIは正直ダサいが、APIがあるなら自分でUIを作ればいい。データさえ取れれば、分析はClaude Codeに任せられる。
...
| Web UI | きれい | 正直ダサい（でもAPIがあれば問題ない） |
