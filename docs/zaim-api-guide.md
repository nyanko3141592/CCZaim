...
| `limit` | int | 取得件数（デフォルト: 20程度、最大: 100程度 ※公式ドキュメント非公開のため実測値） |
| `page` | int | ページ番号 |

**レスポンス例（`mapping=1` 指定時）:**

`mapping=1` を指定すると `category_name` / `genre_name` 等が展開されます。未指定の場合は ID のみ返ります。
...
主なデフォルト支出カテゴリ（ユーザーがカスタムカテゴリを追加している場合は異なります）:
- 食費
- 日用品
- 交通費
- 通信費
